...
# Leftshift 
    ...
    pt = point
    point = float(input("Enter the points for leftshift (as float). If good, enter 0"))
...
ndim = udic['ndim']-1
np.savetxt(path+'T1X.dat',out,
           header=nu.header('2H', str(nu.aqcalc(udic)),str(len(data[0,:])), str(udic[ndim]['obs']), str(udic[ndim]['sw']),
                            str(udic[ndim]['car']), str(pt), str(final_p00), str(p11), str(lb)))