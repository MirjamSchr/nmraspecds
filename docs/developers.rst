...
Welcome to the developer documentation of the nmraspecds package. Unlike the :doc:`API documentation <api/index>`, this part gives some general background information for developers who want to actively contribute to the project.
...
To create a new virtual python environment, open a terminal and change to a a directory where the virtual environment should reside. Then type something like::
  ...
  virtualenv nmraspecds

or alternatively::

  python3 -m venv nmraspecds

This will create a virtual environment in the directory "nmraspecds". To activate this virtual environment, use::
...
The nmraspecds package follows good practice of the Python community regarding directory layout. As there will be several subpackages available, these reside each in a separate directory containing its own ``__init__.py`` file. All packages/modules reside below the ``nmraspecds`` directory of the project root. The ``tests`` directory follows the same structure and contains all the module tests. Generally, the nmraspecds package should be developed test-driven (test-first) as much as possible.
...
The Docstring format used within the code of the nmraspecds package is "NumPy". For convenience, set your IDE accordingly.
...
Developing the nmraspecds package code should be done test-driven wherever possible. The tests reside in the ``tests`` directory in the respective subpackage directory (see above).
...
Static code analysis can be performed using `Prospector <http://prospector.landscape.io/en/master/>`_. First, install the necessary tools into the virtual environment created for the nmraspecds package::
...
For better readability, the prospector output can get printed into a file.
The text output is the most human-readable in my opinion::
